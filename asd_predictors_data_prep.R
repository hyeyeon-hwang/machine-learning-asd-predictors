...
# library(tidyverse)
source("asd_predictors_cb_data_prep.R")
...
dmrFull$plac <- read.delim("../data/Individual/sig_individual_smoothed_DMR_methylation.txt")
...
#' @import tidyverse
#' @export prepData
prepData <- function(dmrFull, sampleInfo, cb = FALSE) {
  if(cb == FALSE) {
    dataPrep1 <- dmrFull %>% 
      as.tibble() %>% 
...
dmr$cb <- prepData(dmrFull$cb, sampleInfo$cb, cb = TRUE)
