...
dmrFullMi <- list()
dmrFullMi$rett <- read.delim("../data/Merged_individual/rett_merged_individual_dmr.txt") #, check.names = FALSE)
dmrFullMi$dup <- read.delim("../data/Merged_individual/dup_merged_individual_dmr.txt")
dmrFullMi$asd <- read.delim("../data/Merged_individual/asd_merged_individual_dmr.txt")
...
sampleInfo$mi <- tibble(Name = c(as.character(sampleInfo$rett$Name), 
...
preCombatPrep <- function(dmrFullMi) {
  data <- dmrFullMi %>%
    as.tibble() %>% 
...
dmrPreCombat <- list()
dmrPreCombat$rett <- preCombatPrep(dmrFullMi$rett)
dmrPreCombat$dup <- preCombatPrep(dmrFullMi$dup)
# remove repeated samples: JLKD063 = 1136 , JLKD066 = 1406, JLKD067 = 1711
dmrPreCombat$asd <- preCombatPrep(dmrFullMi$asd) %>% select(-c("JLKD063", "JLKD066", "JLKD067"))

# joinedCb for combat  
dmrPreCombat$joinedMi <- dmrPreCombat$rett %>%
...
dmrPreCombat$joinedMi <- dmrPreCombat$joinedMi %>%
  add_column(diagnosis = as.factor(sampleInfo$mi$Diagnosis[match(dmrPreCombat$joinedMi$sampleID, sampleInfo$mi$Name)]), .after = 1) %>%
  add_column(batch = as.numeric(sampleInfo$mi$batch[match(dmrPreCombat$joinedMi$sampleID, sampleInfo$mi$Name)]), .after = 2) %>%
  add_column(sample = as.integer(1:nrow(dmrPreCombat$joinedMi)), .before = 1)
groupedDiagnosis <- as.character(dmrPreCombat$joinedMi$diagnosis)
groupedDiagnosis[which(groupedDiagnosis != "Control")] <- "Positive"

dmrPreCombat$joinedMi <- dmrPreCombat$joinedMi %>% 
  add_column(groupedDiagnosis = as.factor(groupedDiagnosis), .after = 1)

modCombatData <- dmrPreCombat$joinedMi[, 1:5]
...
library(sva)
# combat function
postCombatJoinedMi = ComBat(dat = datCombat, batch = batch, mod = modCombat, par.prior = TRUE, prior.plots = FALSE)
#View(postCombatJoinedCb)
#write.table(postCombatJoinedCb, "postCombatJoinedCb.txt", sep = "\t")

# dmrFull <- list(), already in ...cb_data_prep.R
dmrFull$mi <- postCombatJoinedMi
sampleInfo$miGrouped <- sampleInfo$mi
sampleInfo$miGrouped$Diagnosis[ which(sampleInfo$miGrouped$Diagnosis != "Control")] <- "Positive"
